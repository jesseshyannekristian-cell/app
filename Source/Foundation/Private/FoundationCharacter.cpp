...
void AFoundationCharacter::BeginPlay()
{
        Super::BeginPlay();

        // Apply the pre-breach loadout chosen in the Site Overseer hub.
        if (UGameInstance* GI = GetGameInstance())
        {
                if (UOverseerProgression* Prog = GI->GetSubsystem<UOverseerProgression>())
                {
                        for (const FName& Id : Prog->GetLoadout())
                        {
                                if (Id == FName(TEXT("keycard_l5")))      { KeycardLevel = FMath::Max(KeycardLevel, 5); }
                                else if (Id == FName(TEXT("keycard_l1")))  { KeycardLevel = FMath::Max(KeycardLevel, 1); }
                                else if (Id == FName(TEXT("kevlar_vest"))) { Health += 50.f; }
                                else if (Id == FName(TEXT("hazmat_suit"))) { Health += 20.f; }
                        }
                }
        }
}
