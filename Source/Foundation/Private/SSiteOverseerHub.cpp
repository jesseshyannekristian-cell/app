...
TSharedRef<SWidget> SSiteOverseerHub::BuildBreachTab()
{
        ...
        // --- Pre-breach loadout ---
        {
                const int32 Cap = Progress.IsValid() ? Progress->GetLoadoutCapacity() : 0;
                const int32 Used = Progress.IsValid() ? Progress->GetLoadout().Num() : 0;
                List->AddSlot().Padding(0, 6, 0, 4)
                [
                        SNew(STextBlock).Font(HubFont(14, TEXT("Bold"))).ColorAndOpacity(HubColors::Cyan)
                        .Text(FText::FromString(FString::Printf(TEXT("PRE-BREACH LOADOUT  (%d / %d slots)  \u2014  equipped gear is carried into the breach"), Used, Cap)))
                ];

                bool bAnyOwned = false;
                for (const FEquipmentData& E : UFoundationDataLibrary::GetEquipment())
                {
                        if (!Progress.IsValid() || !Progress->IsOwned(E.Id)) continue;
                        bAnyOwned = true;
                        const bool bIn = Progress->IsInLoadout(E.Id);
                        const FName Id = E.Id;
                        List->AddSlot().Padding(0, 0, 0, 4)
                        [
                                MakeRow(E.Name, E.Description, bIn ? TEXT("EQUIPPED") : TEXT(""),
                                        bIn ? TEXT("REMOVE") : TEXT("EQUIP"), true,
                                        bIn ? HubColors::Green : HubColors::Gray,
                                        FOnClicked::CreateSP(this, &SSiteOverseerHub::DoToggleLoadout, Id))
                        ];
                }
                if (!bAnyOwned)
                {
                        List->AddSlot().Padding(0, 0, 0, 6)
                        [
                                SNew(STextBlock).Font(HubFont(12)).ColorAndOpacity(HubColors::Dim)
                                .Text(LOCTEXT("NoGear", "No equipment owned yet. Research & purchase gear in the Research/Store tabs to build a loadout."))
                        ];
                }
                List->AddSlot().Padding(0, 6, 0, 10)[ SNew(SSeparator).Orientation(Orient_Horizontal) ];
        }
...
FReply SSiteOverseerHub::DoToggleLoadout(FName EquipId)
{
        if (Progress.IsValid()) { Progress->ToggleLoadout(EquipId, StatusMsg); }
        RefreshContent();
        return FReply::Handled();
}
