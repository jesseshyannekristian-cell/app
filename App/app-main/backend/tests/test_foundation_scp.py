...
These tests cover the refactor:
...
Note: `is None` / `is not None` comparisons are intentional and correct (singleton
identity checks); only boolean singletons are asserted via truthiness.
"""
import json
import os
import subprocess
...
class TestCliSmoke:
    ...
    def _run_cli(self, stdin_text: str, timeout: int = 15) -> "subprocess.CompletedProcess[str]":
    ...
    def test_launch_and_quit(self) -> None:
    ...
    def test_fresh_save_file_created(self) -> None:
        self._run_cli("\nq\n")
...
# ====================== RNG ======================
class TestRng:
    def test_roll_within_bounds(self) -> None:
        for _ in range(200):
            v = rng.roll(1, 100)
            assert 1 <= v <= 100
        for _ in range(50):
    ...
    def test_chance_distribution(self) -> None:
...
class TestGameStateActions:
    def test_research_rank_lock_at_start(self) -> None:
        s = GameState()
        ok, msg = s.try_complete_research("res_kevlar_vest")
        assert not ok
        assert "rank" in msg.lower()

    def test_research_succeeds_when_eligible(self) -> None:
        s = GameState()
    ...
    def test_research_blocks_duplicate(self) -> None:
        s = GameState()
        s.rank_level = 2
    ...
    def test_store_buy_deducts_credits(self) -> None:
        s = GameState()
    ...
    def test_store_blocks_without_research(self) -> None:
        s = GameState()
    ...
    def test_op_dispatch_updates_currencies_or_xp(self) -> None:
        s = GameState()
...
# ====================== Loadout ======================
class TestLoadout:
    def test_toggle_requires_ownership(self) -> None:
        s = GameState()
        ok, msg = s.toggle_loadout("flashlight")
        assert not ok
        assert "own" in msg.lower()

    def test_toggle_add_then_remove(self) -> None:
        s = GameState()
    ...
    def test_loadout_capacity(self) -> None:
        s = GameState()
...
# ====================== Breach ======================
class TestBreach:
    def test_assess_clamps_and_formula(self) -> None:
        s = GameState()
    ...
    def test_assess_with_loadout_bonus(self) -> None:
        s = GameState()
    ...
    def test_assess_clamp_low_and_high(self) -> None:
        s = GameState()
    ...
    def test_record_breach_result_success(self) -> None:
        s = GameState()
    ...
    def test_record_breach_result_failure(self) -> None:
        s = GameState()
...
class TestArchives:
    def test_add_creates_entry(self) -> None:
        ...
        assert entry["id"] == 1
        assert entry["is_breachable"]
        assert os.path.exists(ARCHIVE_PATH)

    def test_add_persists_on_reload(self) -> None:
        a = Archives.load()
        a.add("SCP-7777", "TEST_Whisper Lamp", "Euclid", "desc", True)
        a2 = Archives.load()
    ...
    def test_update_changes_fields(self) -> None:
        a = Archives.load()
        a.add("SCP-7777", "TEST_Whisper Lamp", "Euclid", "desc", True)
        a.update(1, name="TEST_Whisper Lamp v2", is_breachable=False)
        reloaded = Archives.load()
        assert reloaded.get(1)["name"] == "TEST_Whisper Lamp v2"
        assert not reloaded.get(1)["is_breachable"]
        assert reloaded.breachable() == []

    def test_delete_removes_entry(self) -> None:
        a = Archives.load()
        a.add("SCP-7777", "TEST_Whisper Lamp", "Euclid", "desc", True)
        assert a.delete(1)
        assert Archives.load().custom_scps == []
    ...
    def test_breachable_appears_in_breach_targets(self) -> None:
...
class TestProcessPersistence:
    def test_state_persists_across_runs(self) -> None:
        ...
        # Run 2: separate subprocess loads + quits; header must show the data.
        env = {**os.environ, "TERM": "xterm", "COLUMNS": "120", "LINES": "45"}
        r = subprocess.run(
            [sys.executable, "-m", "game.main"],
            input="\nq\n", cwd=APP_DIR, env=env,
        ...
        assert "1234" in r.stdout  # credits in header
        assert "567" in r.stdout   # research credits in header

        # Reload in this process must show identical data.
        s2 = GameState.load()
    ...
    def test_archives_persist_across_runs(self) -> None:
        a = Archives.load()
