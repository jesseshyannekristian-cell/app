...
export default function Breach({ navigation }) {
  ...
  const start = (t) => {
    sfx.alarm();
    setTarget(t);
    const a = breachAssess(boot, state, t.threat, t.number);
    setChance(a.chance); setAssessLines(a.lines); setLog([]); setOutcome(null); setPhase('assess');
  };
          ...
          <Card title="Pre-Breach Assessment" accent={C.amber}>
            ...
            {assessLines.length ? (
              <View style={{ marginTop: 8 }}>
                {assessLines.map((l, i) => (
                  <Text key={i} style={[F.small, { color: C.green }]}>◦ {l}</Text>
                ))}
              </View>
            ) : (
              <Text style={[F.small, { marginTop: 8, color: C.dim }]}>No relevant procedures researched for {target.number} — containment relies on loadout and rank alone.</Text>
            )}
          </Card>
